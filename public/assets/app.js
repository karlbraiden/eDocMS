...
function openCamBranch(){
        if ($('#toggleButCamRoyal').prop('checked')){
          ...
          if (!brvalPass){
            $('#passapproyal').show(); brvalPass = true;$('#app').hide();brscanner.stop();brtogglecam = false;
          }
        }
}
...
function submitQRPass(content){
  var hash = new Hashes.SHA512().b64(content);
  var todo = {filename:$('#fileroute').val(),monitfile:$('#lbltmp').val(),user:getCookie('me'),hashval:hash, action:$('#routeselAct').val(),remark:$('#routeRemark').val(), branch:$('#routeselBr').val(),subject:$('#routeSubject').val()};
  $.ajax({
    type: 'POST',
    url: '/scancode',
    data: todo,
