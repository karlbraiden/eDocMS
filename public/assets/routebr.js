...
function scanDoc(){
  ...
  if (window.location.toString().includes("/release/")){
    ...
    $('#docArchive').show();$('#routebutBr').hide();
  } else {
    $('#docArchive').hide();$('#routebutBr').show();
...
$('#docArchive').on('click', function(event){
  ...
  var fileroute = $('#fileroute');
  var newfile = $('#newfile');
  var todo = {save:'archive', fileroute: fileroute.val(), newfile:newfile.val()};
